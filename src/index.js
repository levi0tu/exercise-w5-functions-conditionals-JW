...
const longestName = (driver, navigator) => {
  if (driver.length > navigator.length) {
    document.getElementById("app").textContent = "The driver has the longest name.";
  } else if (navigator.length > driver.length) {
    document.getElementById("app").textContent = "The navigator has the longest name.";
  }
  else {
    document.getElementById("app").textContent = "The driver and the navigator has equally long names";
  }
};
longestName("jennie", "margareta")
...
if (userAge > "40") { alert(`Wow that´s old`) }
else { alert(`Wow that´s young`) }
...
const calculationTest = () => {
  const correctAnswer = 8 * 16;
  const userAnswer = prompt("What is 8 * 16?");
  if (parseInt(userAnswer) === correctAnswer) {
    alert(`Yippie! Correct`);
  } else {
    alert(`Noo, the correct answer is ${correctAnswer}`);
  }
};
calculationTest();
/*
